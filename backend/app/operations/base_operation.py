...
class BaseOperation(BaseHandler):
    ...
    async def select_calendar(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        """Select appropriate calendar for event. Uses calendar_suggestion from combined prompt."""
        ...
        # Check for user-specified calendar name
        calendar_name = event_data.get('calendar_name')
        if calendar_name and self.calendar_agent:
            calendar_id = self.calendar_agent.find_calendar_by_name(calendar_name)
            if calendar_id:
                logger.info(f"📅 USER CALENDAR: Using '{calendar_name}' -> {calendar_id}")
                return {
                    "success": True,
        ...
        logger.info(f"📅 DEFAULT CALENDAR: Using primary calendar")
        return {
            "success": True,
            "calendar_id": "primary",
